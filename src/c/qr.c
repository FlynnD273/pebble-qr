...
uint8_t gen_qr(char *str, uint8_t **cells) {
  ...
  uint8_t size = get_size(str, str_len);
  if (!size) {
  ...
  uint8_t version_idx = size_to_idx(size);
  ...
  // 0b----------------5678
  data[2] = (str_len >> 8) & 0x0F;
  for (size_t i = 0; i < str_len; i++) {
    data[i + 2] = (data[i + 2] & 0xF0) | str[i] >> 4;
    data[i + 3] = str[i] << 4;
  }
  for (size_t i = 0; i < req_byte_len - str_len; i++) {
    if (i % 2) {
      data[i] = 0b00010001;
    } else {
      data[i] = 0b11101100;
  ...
  APP_LOG(APP_LOG_LEVEL_DEBUG, "cells: %p", tmp_cells);
  APP_LOG(APP_LOG_LEVEL_DEBUG, "reserved: %p", reserved_mask);
  uint8_t *masks = gen_masks(tmp_cells, reserved_mask, size);
  uint16_t scores[8];
  for (uint8_t i = 0; i < 8; i++) {
    APP_LOG(APP_LOG_LEVEL_DEBUG, "Eval %d", i);
  ...
  *cells = (uint8_t *)malloc(len);
  ...
  APP_LOG(APP_LOG_LEVEL_DEBUG, "format str");
  uint16_t format_str = 0b01 << 3;
  format_str |= min_idx;
  format_str <<= 10;
  uint16_t err_str = format_str;

  uint8_t err_str_len = bin_len(err_str);
  while (err_str_len > 10) {
    uint16_t generator = 0b10100110111;
    uint8_t gen_str_len = bin_len(generator);
    generator <<= err_str_len - gen_str_len;
    err_str ^= generator;
    err_str_len = bin_len(err_str);
  }
  format_str |= err_str;
  format_str ^= 0b101010000010010;

  APP_LOG(APP_LOG_LEVEL_DEBUG, "write format str");
  for (uint8_t i = 0; i < 14; i++) {
    if (i < 7) {
      if (i < 6) {
        set_module(*cells, size, i, 8, 1);
      } else {
        set_module(*cells, size, i + 1, 8, 1);
      }
      set_module(*cells, size, 8, size - i, 1);
    } else {
      if (i < 9) {
        set_module(*cells, size, 8, 8 - i - 8, 1);
      } else {
        set_module(*cells, size, 8, 8 - i - 8 + 1, 1);
      }
      set_module(*cells, size, size - 8 - i - 8, 8, 1);
    }
  }

  if (version_idx >= 6) {
    APP_LOG(APP_LOG_LEVEL_DEBUG, "version str");
    uint16_t version_str = 0b01 << 3;
    version_str |= min_idx;
    version_str <<= 13;
    err_str = version_str;

    err_str_len = bin_len(err_str);
    while (err_str_len > 12) {
      uint16_t generator = 0b10100110111;
      uint8_t gen_str_len = bin_len(generator);
      generator <<= err_str_len - gen_str_len;
      err_str ^= generator;
    ...
    for (uint8_t long_i = 0; long_i < size; long_i++) {
      for (uint8_t short_i = 0; short_i < size; short_i++) {
        uint8_t val = (version_str >> i++) & 1;
        if (val) {
          set_module(*cells, size, long_i, size - 11 + short_i, 1);
          set_module(*cells, size, size - 11 + short_i, long_i, 1);
        }
      }
    }
  }
...
uint8_t *gen_masks(uint8_t *cells, uint8_t *reserved_mask, uint8_t size) {
  APP_LOG(APP_LOG_LEVEL_DEBUG, "create mask pointers");
  uint16_t len = ((uint16_t)size * size + 7) / 8;
...
uint16_t eval_board(uint8_t *cells, uint8_t size) {
  ...
  APP_LOG(APP_LOG_LEVEL_DEBUG, "step 1");
  ...
  APP_LOG(APP_LOG_LEVEL_DEBUG, "step 2");
  ...
  APP_LOG(APP_LOG_LEVEL_DEBUG, "step 3");
  ...
  APP_LOG(APP_LOG_LEVEL_DEBUG, "step 4");
  ...
  APP_LOG(APP_LOG_LEVEL_DEBUG, "done eval");
...
uint8_t size_to_idx(uint8_t size) { return (size - 21) / 4; }
...
uint8_t *get_ec(uint8_t *data, uint8_t data_len, uint8_t codewords) {
  ...
  uint8_t *ret = poly_divide(messagePoly, codewords, gen_poly, degree + 1);
  free(messagePoly);
  free(gen_poly);
  return ret;
}
...
uint8_array interleave(uint8_t *data, uint8_t **group1, uint8_t **group2,
  ...
  size_t ec_group1_size = group1_block_size * group1_block_count;
  size_t data_group2_size = group2_block_size * group2_block_count;
  size_t ec_group2_size = group2_block_size * group2_block_count;
  arr.len =
